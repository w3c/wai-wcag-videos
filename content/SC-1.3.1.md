...
| 5 | @:@@ - @:@@ | Yet this only works when the page structures are coded correctly. Otherwise the entire page is one large block of text without any structural information, which is unusable for Ilya and many others. | We see a large block of text morph into HTML code with structures (indicated by spacing and indents in the code). We do not see the exact text/code, just that it changes from unstructured to structured. |
